...
- Treat API Gateway propagation after a Lambda alias cutover as eventually consistent: post-cutover CORS verification must use a bounded retry and assert each route's required methods and headers. Keep pre-update compatibility checks explicit so a previously deployed Worker is not required to satisfy a newly introduced route contract before it is replaced.
