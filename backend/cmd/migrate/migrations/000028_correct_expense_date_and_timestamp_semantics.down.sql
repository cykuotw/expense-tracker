...
ALTER TABLE invitations
    ...
    ALTER COLUMN created_at TYPE timestamp without time zone
        USING created_at AT TIME ZONE 'UTC',
    ALTER COLUMN registration_session_expires_at TYPE timestamp without time zone
        USING registration_session_expires_at AT TIME ZONE 'UTC';

ALTER TABLE balance
    ALTER COLUMN create_time_utc TYPE timestamp without time zone
        USING create_time_utc AT TIME ZONE 'UTC',
    ALTER COLUMN update_time_utc TYPE timestamp without time zone
        USING update_time_utc AT TIME ZONE 'UTC',
    ALTER COLUMN settle_time_utc TYPE timestamp without time zone
        USING settle_time_utc AT TIME ZONE 'UTC';

ALTER TABLE expense
    ALTER COLUMN delete_time_utc TYPE timestamp without time zone
