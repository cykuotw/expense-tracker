...
func TestValidateStatusChange(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := validateStatusChange(test.actor, test.target, test.role, test.current, test.next, test.activeAdmins)
...
func TestValidateRoleChange(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := validateRoleChange(test.actor, test.target, test.currentRole, test.nextRole, test.active, test.activeAdmins)
			assert.True(t, errors.Is(err, test.expected))
		})
	}
}
