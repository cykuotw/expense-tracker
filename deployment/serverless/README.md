...
- Post-cutover API verification allows up to 60 seconds for API Gateway and Lambda alias propagation. It verifies the OCR draft preflight specifically permits `POST` plus `Content-Type`, `Authorization`, `X-OCR-Account-ID`, and `X-OCR-Request-ID`; the pre-update health gate deliberately defers this new contract until the Worker has been promoted.
