...
self.addEventListener("push", (event) => {
    ...
    event.waitUntil(self.registration.showNotification(payload?.title ?? "Expense Tracker", { body: payload?.body ?? "New activity is available", tag: payload?.tag ?? "expense-tracker-activity", data: { url: payload?.url ?? "/" } }));
});
self.addEventListener("notificationclick", (event) => {
    event.notification.close();
    const destination = notificationDestination(self.location.origin, event.notification.data?.url);
